...
if not os.path.exists(model_path):
    logger.error(
        ...
        "See README for instructions: https://github.com/kramli436-tech/RAMURI/blob/main/README.md"
    )
    sys.exit(1)
...
def predict(image):
    ...
    output = np.squeeze(output)  # type: ignore[arg-type]
    
    # Ensure output is 2D: if only 1 detection, squeeze makes it 1D
    if output.ndim == 1:
        output = output[np.newaxis, :]
    
    detections = []
    for r in output:
        if len(r) >= 5 and r[4] > conf:
...
def get_positions(image_input):
    ...
    try:
        ...
        logger.error(f"Error during board detection prediction: {e}", exc_info=True)
        return None
