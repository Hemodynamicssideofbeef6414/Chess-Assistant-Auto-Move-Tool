...
def _handle_dev_resources(relative_path: str) -> str:
    ...
    dev_base = Path(__file__).parent.parent  # src/
    dev_path = dev_base / relative_path
    
    # If not found in src/, check project root (parent of src/)
    if not dev_path.exists():
        project_root = dev_base.parent / relative_path
        if project_root.exists():
            logger.debug(f"Dev resource path for '{relative_path}': {project_root}")
